...
class DefaultExtension extends MProvider {
  ...
  async getList(slug) {
    ...
    var list = [];
    var hasNextPage =
      body.selectFirst("a.next.page-numbers").text.length > 0 ? true : false;
  ...
  async getDetail(url) {
    ...
    var status = this.statusFromString(
      infos.at(statusIndex).selectFirst("a").text
    );
    var genre = [];
    infos
      .at(-1)
      .select("a")
      .forEach((a) => genre.push(a.text.trim()));
  ...
  async splitStreams(streams, server) {
    ...
    if (server == "Asianload") {
      ...
      hostUrl = autoStreamUrl.substring(
        0,
        autoStreamUrl.indexOf("master.m3u8")
      );
    }
