const mangayomiSources = [
  {
    ...
    "apiUrl": "",
    "iconUrl":
      "https://www.google.com/s2/favicons?sz=128&domain=https://gojo.wtf/",
    "typeSource": "multi",
    "itemType": 1,
    "version": "0.0.6",
...
class DefaultExtension extends MProvider {
  getHeaders() {
    return {
      ...
      "User-Agent":
        "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/92.0.4084.56 Safari/537.3",
    };
  }
  ...
  async search(query, page, filters) {
    ...
    var res = await this.gojoAPI(
      `/search?query=${query}&page=${page}&perPage=30`
    );
    if (res != null) {
      ...
      for (var i = 0; i < maxEpisodes; i++) {
        ...
        for (var prd of body) {
          ...
          if (chap.hasOwnProperty("isFiller")) {
            if (
              !chapScan.includes("Filler") &&
              chap.isFiller &&
              this.getPreference("gojo_pref_mark_filler")
            ) {
        ...
        chapters.push({
          name: chapName,
          url: `${anilistId}||` + JSON.stringify(chapLink),
          scanlator: chapScan,
        });
      }
    }
