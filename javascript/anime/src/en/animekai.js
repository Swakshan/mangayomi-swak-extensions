...
class DefaultExtension extends MProvider {
  ...
  async getDetail(url) {
    ...
    var chapters = [];
    var ratingField = body.selectFirst("#anime-rating");
    var animeId = ratingField.attr("data-id");
    var anilist_id = ratingField.attr("data-alid");
    ...
    if (body.status == 200) {
      ...
      var additionalDetails = await this.getEpisodeDetails(anilist_id);
      var showEpThumbnail = this.getPreference("animekai_pref_ep_thumbnail");
      var showEpDescription = this.getPreference(
        "animekai_pref_ep_description"
      );
  ...
  getSourcePreferences() {
    return [
      ...
      {
        key: "animekai_pref_ep_description",
        switchPreferenceCompat: {
  ...
  // ------------------------------
  async getEpisodeDetails(malId) {
    var addInfoApi = `https://api.ani.zip/mappings?anilist_id=${malId}`;
